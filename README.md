...
The condition: **every custom content model namespace must be listed in the Community indexer's
namespace prefix map.** If it is not, nodes are silently dropped or silently stripped of their
custom properties, with nothing in the dead letter queue to tell you. See
...
| `custom-model-prefix` | PASS | Measures the cost of an unconfigured namespace, and that adding it recovers everything. |
...
was still `2102590253222` and the name was still the planted one, so the batch writer read the
...
The repository maps custom model fields by itself. The indexer resolves a node's type and property
names through a static prefix map baked into its image, and that map only knows the namespaces
Alfresco ships. The harness deploys a model in `http://parity.example.org/model/1.0` and indexes
two shapes of node with the stock map:
...
- A node whose own **type** is custom (`parity:record`) was **not indexed at all**. No document,
  ...
  `impossible to retrieve type name for node 874` and counted it as a filtered item.
- An ordinary `cm:content` node merely **carrying** a custom aspect was indexed, but without
  `parity%3Aclassification`, and its `ASPECT` field was reduced to `["cm:auditable"]`. The custom
  aspect had vanished from the document.

Adding one line to the prefix map, restarting the indexer and touching both nodes recovered
everything: the typed node appeared with `TYPE: parity:record` and all 5 custom fields, and the
aspect node came back with `ASPECT: ["parity:classified", "sys:cascadeUpdate", "cm:auditable"]`
and its classification value.
...
The map is a JSON resource inside the indexer image. The harness reads it out of the pinned image
at run time and mounts an extended copy, overriding `alfresco.reindex.prefixes-file` as a JVM
system property because it is consumed through a Spring `@PropertySource` and an environment
...
python3 -m harness prepare                    # write generated/prefixes.json only
...
1. List every namespace URI of every custom model deployed in the repository, and confirm each one
   is in the indexer's prefix map. This is the only issue the harness found that can lose data.
2. If any namespace was missing, the affected nodes need reindexing after you fix the map. Nothing
   in the index tells you which they are.
...
harness/                         the checks, as a Python package with no dependencies
  ...
  stack.py                       Compose driver, and the prefix map read from the image
...
extracted from the pinned image at run time.
